...
- If `key` is not sent, user is created as `worker`.
- If `key` is sent and matches backend `ADMIN_KEY`, user is created as `admin`.
- If `key` is sent and does not match backend `ADMIN_KEY`, request fails with validation error (`message: "Key fail"`).
...
- Method: `PATCH` (preferred), `POST` (supported)
...
Batch request body (recommended for scanner flow):
...
- Backend accepts both `PATCH /shopify/items/location` and `POST /shopify/items/location` with the same payload and response shapes.
...
### Append Metafield Options (Admin only)
...
- Trims every option value.
- Removes duplicates in incoming values.
- Merges incoming values into existing metafield choices.
- Does not require sending the full existing list.
...
### Remove Metafield Option (Admin only)

- Method: `DELETE`
- Path: `/shopify/metafields/options/:optionValue`
- Auth required: `Yes`
- Admin role required: `Yes`
- Shop link required: `Yes`
- Header: `Authorization: Bearer <accessToken>`
...
- `optionValue` required string (URL-encoded when needed)

Behavior:

- Removes a single option value from `app.item_location` choices.
- Returns `VALIDATION_ERROR` if option does not exist.
