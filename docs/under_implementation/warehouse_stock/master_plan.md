...
| P5 report | **APPROVED (rounds 1 + 2)** | 2026-09-02 | coordinator | gate **SATISFIED**: P3 APPROVED 2026-09-02 (shares `stock.contract.ts`/`stock.controller.ts`/`stock.routes.ts`) and the intention header reads RATIFIED. **Plan rewritten** earlier under intention §26 (owner-approved report amendment): 6 criteria → 3, contract §4.7 at v1.2. Lint **PASS after 4 folds** → `prompts/coordinator/plan_lint_5.md`: L1 §6.4's instrument row still described the pre-§26 phase ("C1–C6 aggregation rows") and pointed the implementer at work the plan forbids; L2 the perimeter omitted `scripts/verify-all.ts` while §6.4 obliges the authoring phase to extend `EXPECTED_SCRIPTS` in the same commit; L3 no derived row count (**14** — C1 4, C2 6, C3 4); L4 **C1(d) was vacuous** — one `Shop` in the database means a fixture seeding only it asserts nothing, so the row now requires a second shop. **Projection round 0 WAIVED by the owner (David), 2026-09-02** — the recorded line §3 requires. Grounds: a pure read with no mutation path, 3 criteria / 14 rows, and its one silent-failure mode (a `mergeKey` that quietly splits or merges the client's groups) already carries six criterion rows and a named planted-defect probe aimed at it; the lint had just cleared the plan's actual defects. **Accepted cost:** projection has surfaced something real in all three phases that ran it — P3's found threshold replacement had no write path anywhere in `src/`, P4's caught a coordinator row-count error — so anything a skeleton would have discovered here must now be caught at review instead. Prompt compiled → `prompts/implementer/prompt_plan5_implement_r1.md`, **gate self-test 5/5** (RATIFIED · branch · P3 present with `toLocationStockDto` exported · own files absent · typecheck 0 + `SUMMARY PASS 2 script(s)`). Implemented `dedf415`; review r1 **APPROVED**, no blocking finding and no fix cycle. Perimeter exact (additive only in the three files shared with P3; `verify-all.ts` limited to the `EXPECTED_SCRIPTS` line); plan file append-only. Instruments: typecheck 0, purity empty, **`SUMMARY PASS 3 script(s)`** — 58 P1 + 20 P2 + **14 P5** — refusal guard exit 3 in all three forms, `prisma/dev.db` untouched. **All 14 rows and all three manual scenarios re-executed by the reviewer** on a live server: a **scalar**-created and an **array**-created teak definition returned the same `mergeKey` on real inventory, a zero-match definition appeared at `0`/`out_of_stock`, a parameterized call was byte-identical to the bare one, both mounts 200, no token 401. **Four reviewer mutations**: key omits `itemCategory` → C2(d) red; key includes `location` → C2(a)(b)(e)(f) red; zero-quantity rows dropped → 8 rows red. **N1: the plan's own named probe reddened nothing** — `propertiesCanonical` and `JSON.stringify(properties)` are provably identical for all reachable data, so the probe was mis-aimed; the failure mode it targeted is covered by C2(d) and C2(a)/(e)/(f). A **plan defect the implementer caught and reported** rather than working around, as P3's did. N2 advisory (C3(d) partly a source-text match). **Round 2 opened 2026-09-02 — additive, not a fix cycle.** The owner asked the report to carry what a definition considers full. §26.1's ratified six-field entry made that a material change: **gate re-opened (round 4), §27 written, owner re-stamped RATIFIED the same day**. Two owner decisions: the target is the `normal_in_stock` threshold (**fill**, so qty 7 with 10/15/20 → **13**, not 9) and the entry carries **both** the three thresholds and the derived count, named `unitsToNormalThreshold` so it cannot be read as "units to become normal". The qty-18 case — already normal, still reports 2 — is deliberate and pinned. Registry §6.5 → eight fields; contract reissued **v1.6** (additive; nothing built on v1.5 changes). Plan gained **C4** (5 rows) → **19**. Lint **PASS after 3 folds** → `prompts/coordinator/plan_lint_5_round2.md`: L1 C4's 10/15/20 collided with the shipped fixture's 1/3/5 (C4 gets its own set; round 1's stays); L2 C4(c)'s "already normal" premise was fabricated unless the state is **derived** not hand-typed; L3 the file list was round 1's. Prompt → `prompts/implementer/prompt_plan5_implement_r2.md`, gate self-test 4/4. **The round-2 probe discriminates** — mutating to the rejected `medium+1` reading reddens C4(a) *and* C4(c) — which is what round 1's probe could not do. Implemented `c65dca7`; review r2 **APPROVED**, phase closed. Perimeter exactly the three permitted files — controller, routes and `verify-all.ts` untouched. `SUMMARY PASS 3 script(s)` with **19** P5 rows; live on real inventory LC1 221 with 200/250/300 → `medium_in_stock` needing **79**, H1 34 → clamped **0**, an empty definition → **20**. Reviewer probes: the plan's `medium+1` mutation reddened **three** rows (C4(a) 13→9, C4(b) 20→16, C4(c) 2→0) — it discriminates, unlike round 1's. **N1: `thresholds[2]` instead of the state lookup reddens nothing** — alphabetical `orderBy` makes index 2 *be* normal by accident, so a regression to index lookup would pass all 19 rows while returning a wrong restock number. Code is correct; the coverage gap is routed to P6 as an **optional** row (§9.7). Lesson: a hazard named in a plan is not a hazard covered by a criterion |
| P7 per-instance count | **IMPLEMENTED** | 2026-09-02 | Claude Fable 5.1 (implementer) | plan `plans/plan_7_per_instance_count.md`, owner cards D1–D6 answered as written, gate satisfied (`4fcbc17` committed). Backend: `instanceCount` column + migration `20260902170346_add_location_stock_instance_count`, `domain/allocation.ts`, both numbers moved by hooks/reconciliation/rebuild, thresholds item-based, report entry nine fields. Frontend: items by default, per-device units switch on the filter sheet and PDF sheet, PDF headers/settings follow the mode. Instruments: backend typecheck 0, purity grep empty, **`SUMMARY PASS 3 script(s)` — 66 + 33 + 25 = 124 rows** (was 97 + the state refactor's additions; P7 added 5 + 13 + 6 = 24 rows), dev.db checksum unchanged; frontend typecheck 0, lint 62 problems = HEAD baseline 62 (stock feature clean), **vitest 209/209** (189 baseline + 20 P7). Named probes executed: 4 backend + 1 frontend, all red where aimed (see plan Review log). Pre-existing P5 C1(a) fixture defect fixed in passing (it assumed an empty shop). Handoff `handoffs/implementer/handoff_7_implement_1.md`. **Owner card open: backfill of the local `prisma/dev.db`** (its three rows still read `instanceCount 0`; the rebuild script refuses that path by P6 design) |
| P6 maintenance + verification sweep | **APPROVED** | 2026-09-02 | coordinator | gate **SATISFIED** (P3, P4, P5 all APPROVED). **C3 DISCHARGED 2026-09-02 by owner attestation** — the owner exercised the endpoints and live functionality against the running system and approved the row on that basis; it is **not** a step-by-step run of `verification/end-to-end-runbook.md`, and C4 must cite it as an attestation, never as a recorded sweep. The runbook stays committed and re-runnable, annotated with its own status. Lint **PASS after 3 folds** → `prompts/coordinator/plan_lint_6.md`: **L1 C5 was unsatisfiable** — it demanded PASS for "every script in the §6.4 table", which has four rows including `verify-all.ts` itself, and the runner excludes itself and never emits a PASS line for itself, so the phase's own close condition could never be met in a file nobody may edit (the exact trap §6.4's earned note records, re-introduced); L2 no derived row count (**9** across 5 criteria); L3 C3's discharge and its boundary were unstated, so an implementer would have tried to run it and the ledger would have conflated attestation with execution. Prompt → `prompts/implementer/prompt_plan6_implement_r1.md`, **gate self-test 5/5** (RATIFIED · branch · P3–P5 approved · own files absent · typecheck 0 + `SUMMARY PASS 3 script(s)` / **97 rows**). **Implementer scope was C1, C2, C5 — 6 rows**; C4 the coordinator's. Implemented `4a0d5ef`; review r1 **APPROVED**. Perimeter exactly the two new scripts, `verify-all.ts`/`EXPECTED_SCRIPTS` untouched. **C5: `SUMMARY PASS 3 script(s)` — 97 rows**, none REFUSED or MISSING; `prisma/dev.db` SHA-256 identical before and after the whole review. C1 executed by the reviewer against the real dev.db (1107 rows, 0 drift, checksum unchanged) and four planted states on a scratch copy — including `"Oak, Bamboo / Teak"` reporting **only** `bamboo`, and the un-required `property-key-unobserved` output firing. **C2: the dry run re-implements P2's group computation, so it was tested, not argued** — corrupting three definitions to 99 gave a preview of 80/107/114 and a live run that wrote exactly 80/107/114, the P3-review figures; `writes: 0` proven by checksum; refusal exit 3 in all three forms. **C4: M1–M8 all verified**, M3 alone resting partly on the owner's attestation, stated not smoothed. N1 advisory (the rebuild duplicates P2's `computeGroup`; proven equivalent, first thing to re-check if allocation semantics change). **Project closed — all six phases APPROVED** |
...
```prisma
enum StockState {          // renamed 2026-09-02 by the owner's state refactor (commit 4fcbc17):
  out_of_stock             //   normal_in_stock -> high_in_stock, high_in_stock -> extra_in_stock
  low_in_stock
  medium_in_stock
  high_in_stock
  extra_in_stock
...
model LocationStock {
  ...
  quantity            Int     @default(0)      // unit sum of the allocated items
  instanceCount       Int     @default(0)      // P7 (2026-09-02): number of allocated items, each counting 1; thresholds/state read THIS
...
| File | Exports |
|---|---|
| `stock-state.ts` | `STOCK_STATES` (`as const`, severity-ascending, single source for sorting §0.20), `StockState`, `CONFIGURABLE_THRESHOLD_STATES` (`["low_in_stock","medium_in_stock","high_in_stock"] as const` — renamed by the owner's refactor `4fcbc17`), `calculateStockState(count, thresholds)` — **P7: every caller passes `instanceCount`, never `quantity`**; `validateThresholds(thresholds)` (one to three rows since `4fcbc17`), `normalizeThresholdInputs(inputs)` |
| `allocation.ts` | **P7 (2026-09-02).** `allocateGroup(candidates, items): Map<id, { quantity, instanceCount }>` — the single allocation loop (best-match per item, both numbers accumulated); `AllocationItem`, `AllocationTotals`, `emptyAllocationTotals()`. Called by `stock-reconciliation.service.ts` and `scripts/rebuild-location-stock.ts`, which closed P6 review N1 (the rebuild's duplicated loop) |
...
| `repositories/location-stock.repository.ts` | `locationStockRepository` object literal; ALL Prisma access; guarded decrement (§0.15); `{}` round-trips as `{}` (§0.21); accepts optional `tx`. **P7:** `applyGuardedDecrement(id, shopId, delta: StockDelta, context)` guards and decrements both columns in one statement; `applyIncrement(id, delta: StockDelta)`; `writeAbsolute(id, { quantity, instanceCount }, stockState, by, tx?)`; `updateState` derives from `instanceCount` |
| `services/stock-reconciliation.service.ts` | `reconcileGroup(shopId, location, itemCategory, hooks?)` double-pass (§0.17 + §23.6), `reconcileAllGroups(shopId, hooks?)`. **P7:** computes both numbers via `allocateGroup`; the changed-row compare is three-valued (`quantity`, `instanceCount`, `stockState`) |
...
|---|---|---|
| `scripts/verify-stock-domain.ts` | P1 | P1 C2–C8 (pure domain; no DB); **P7 C1(a)–(e)** as `P7.C1(*)` rows |
| `scripts/verify-stock-reconciliation.ts` | P2 | P2 C1–C6 (scratch DB copy); **P7 C2(a)–(e), C3(a)–(h)** as `P7.C2(*)`/`P7.C3(*)` rows |
...
LocationStockDto = { id, location, itemCategory, properties: StockCriteria (canonical),
  quantity, instanceCount, stockState, thresholds: [{ state, thresholdQuantity }],   // instanceCount: P7
  createdAt, createdByUsername, updatedAt, updatedByUsername }

StockDelta = { quantity: number; instances: number }   // P7: one movement; same-definition edits carry instances: 0
...
StockReportDto = { entries: [{ location, itemCategory, properties, mergeKey, quantity, instanceCount, stockState,
                               thresholds: [{ state, thresholdQuantity }], unitsToRestockTarget }] }
  // intention §26, entry shape extended by §27 (ratified 2026-09-02); P7 adds instanceCount. NINE fields.
  // P7 (2026-09-02): stockState and unitsToRestockTarget are derived from instanceCount, never quantity.
  // unitsToRestockTarget (renamed from unitsToNormalThreshold by the owner's refactor 4fcbc17)
  //   = max(0, highest configured threshold − instanceCount).
