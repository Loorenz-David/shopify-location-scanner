...
| C4 | Ledger closeout row per measurement: M1–M8 each marked verified with a pointer to the evidence (P1–P5 Review logs or C3 steps). M6→P1-C8/P3-C1–C2, M7→P5 sweep, M8→the catch-all/no-catch-all totals check run once each way | M1–M8, §24 |

Phase-close instruments: typecheck green; purity grep empty; perimeter diff (2 files).
