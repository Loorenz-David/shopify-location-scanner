...
- `instanceCount` — **items** currently allocated: the number of scanned items (ScanHistory rows) the definition counts, each item counting 1 whatever its quantity (v1.7),
- `quantity` — **units** currently allocated: the sum of those items' quantities (maintained automatically by the backend as items are scanned, moved, sold, returned, or edited in Shopify),
- one to three user-set thresholds (`low_in_stock`, `medium_in_stock`, `high_in_stock`, strictly increasing among those configured; v1.7 — was "three, mandatory"),
- a derived `stockState`.

**Thresholds count items, not units (v1.7).** `stockState` and `unitsToRestockTarget` are derived from `instanceCount`; `quantity` never enters that arithmetic. A definition holding one item of quantity 40 under thresholds 10/15/20 is `low_in_stock`, not `extra_in_stock`.

**States, severity-ascending (this exact order drives all sorting and any severity UI):**
`out_of_stock → low_in_stock → medium_in_stock → high_in_stock → extra_in_stock`.
Semantics with thresholds low=10 / med=15 / high=20, counted in items: `0 → out_of_stock`, `1–10 → low`, `11–15 → medium`, `16–20 → high`, `>20 → extra`. A state whose threshold is not configured is skipped: with only `low=10`, `1–10 → low`, `>10 → extra`.
...
{ "data": [ {
  ...
  "quantity": 29, "instanceCount": 12, "stockState": "medium_in_stock",
  "thresholds": [
...
```
This `LocationStockDto` is the shape everywhere a definition is returned. **`instanceCount` (v1.7)** is the item count the state is derived from; `quantity` is the unit sum (12 items totalling 29 units above → `medium_in_stock` because 12 sits in 11–15). `thresholds` carries only the configured rows — one to three of them.
...
{ "data": { "entries": [
  { "location": "LC1", "itemCategory": "Dining Chairs",
    ...
    "mergeKey": "<opaque>", "quantity": 4, "instanceCount": 2, "stockState": "low_in_stock",
    "thresholds": [
      { "state": "low_in_stock",    "thresholdQuantity": 10 },
      { "state": "medium_in_stock", "thresholdQuantity": 15 },
      { "state": "high_in_stock",   "thresholdQuantity": 20 } ],
  ...
  { "location": "H1",  "itemCategory": "Dining Chairs",
    "properties": { "wood_type": ["walnut"] },
    "mergeKey": "<same opaque value>", "quantity": 6, "instanceCount": 3, "stockState": "low_in_stock",
    "thresholds": [
      { "state": "low_in_stock",    "thresholdQuantity": 10 },
      { "state": "medium_in_stock", "thresholdQuantity": 15 },
      { "state": "high_in_stock",   "thresholdQuantity": 20 } ],
...
**Nine fields per entry (v1.7):** `location`, `itemCategory`, `properties`, `mergeKey`, `quantity`, `instanceCount`, `stockState`, `thresholds`, `unitsToRestockTarget`. All shipped.

**`instanceCount`** (v1.7) is the number of items allocated to the definition; **`quantity`** is their unit sum. Show `instanceCount` by default — it is the number the thresholds are about — and offer `quantity` as an alternative view. Whichever you show, the gap below is in items.

**`thresholds`** is the definition's configured bands, **one to three rows** — the **same shape** §4.3
returns for a configuration, so you can reuse whatever renders it there. Look the row you want up
**by `state`**, never by array position, and never assume a row exists.

**`unitsToRestockTarget`** (renamed from `unitsToNormalThreshold`; basis changed in v1.7) is how many **items** this definition needs to be **full**:

```
restockTarget        = highest configured thresholdQuantity
unitsToRestockTarget = max(0, restockTarget − instanceCount)
```

The name keeps the word "units" for wire stability; the number is a count of items. It is computed on the backend on purpose. The band boundaries are domain rules, and the same
reasoning that keeps `mergeKey` server-side applies here — a client that re-derives them will
eventually derive them differently. Display it; don't recompute it — and if you ever fall back to
computing it, subtract `instanceCount`, not `quantity`.

**Read this table before you render it. The third row looks like a bug and is not:**

| `instanceCount` | `quantity` | `stockState` | `unitsToRestockTarget` | what to show |
|---|---|---|---|---|
| 0 | 0 | `out_of_stock` | **20** | bring a full shelf |
| 7 | 30 | `low_in_stock` | **13** | **not 9** — 9 would only *enter* the high band; this number fills it. And **not 0**: 30 units would look "full" under a unit reading, but thresholds count items |
| 18 | 36 | `high_in_stock` | **2** | **already `high_in_stock`, and still asking for 2.** Deliberate (owner decision, intention §27.3): high is a band of `16–20`, and 18 is inside it but not yet full. If your UI hides the number whenever the state is high, you will hide a legitimate restock. |
| 25 | 50 | `extra_in_stock` | **0** | never negative — safe to render unconditionally |
| 4 | 9 | `low_in_stock` | **6** | only `low_in_stock: 10` configured → the target is 10 |

**Why "fill" and not "clear the warning".** With thresholds 10/15/20 the states are
`0 → out`, `1–10 → low`, `11–15 → medium`, `16–20 → high`, `>20 → extra`. A definition *enters*
high at 16 and *fills* it at 20. The owner chose the fill number, so it is a replenishment
target, not the minimum that stops the warning.
...
**A definition with `instanceCount: 0` is included**, always. It is the report's most urgent signal.
...
**Status as of 2026-09-02 (v1.7)** — P1–P6 approved and the project closed; **P7 (per-instance
count) is implemented and awaiting review.** Everything below describes shipped behaviour; items
3–4 are kept as history.
...
11. **Contract v1.7 (P7: `instanceCount` on §4.3 and §4.7; thresholds, state and gap item-based; plus the state-refactor renames)** — landed with P7's implementation 2026-09-02. Frontend built in the same change: items by default, per-device switch to units on the report and the PDF sheet.
10. ~~Contract v1.6 (report entry gains `thresholds` + `unitsToNormalThreshold`)~~ — landed; superseded by v1.7's rename to `unitsToRestockTarget`. **Additive to v1.5**: nothing you built against v1.5 changes. It is **self-contained**: it is the only file you need for this integration, and every amendment is explained where it applies rather than in a companion notice. The mocks encoding the request case's §3 shape are now authoritative-matching and can be pointed at the real endpoint once P5 is approved.
