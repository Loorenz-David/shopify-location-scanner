...
export const LOGISTIC_INTENTION_LABELS = {
    customer_took_it: "Customer Took It",
    store_pickup: "Store Pickup",
    local_delivery: "Local Delivery",
    international_shipping: "International",
};
export const LOGISTIC_INTENTION_ORDER = [
    "store_pickup",
    "local_delivery",
    "international_shipping",
    "customer_took_it",
];
export function normalizeLogisticTaskItem(dto) {
    return {
        id: dto.id,
...
export function normalizeLogisticTasksPage(dto) {
    const groups = dto.orders.map((orderGroup) => ({
...
export function flattenOrderGroups(groups) {
    return groups.flatMap((g) => g.items);
}
export function buildOrderGroups(items) {
...
export function groupByIntention(items) {
    const map = new Map();
    for (const item of items) {
...
export function countByIntention(items) {
    const counts = {};
    for (const item of items) {
        if (item.intention === null)
            continue;
...
export function buildFiltersFromRoleDefaults(defaults) {
    const filters = {};
...
export function buildApiQueryParams(filters, q) {
...
export function itemMatchesFilters(item, filters) {
    if (filters.noIntention === true && item.intention !== null)
        return false;
