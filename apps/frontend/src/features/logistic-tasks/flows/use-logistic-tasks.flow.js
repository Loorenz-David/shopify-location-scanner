...
import { selectLogisticTasksFiltersRequestKey, selectLogisticTasksHasLoaded, useLogisticTasksStore, } from "../stores/logistic-tasks.store";
const SEARCH_DEBOUNCE_MS = 300;
const INITIAL_LOADING_VISIBILITY_DELAY_MS = 180;
const REFRESH_LOADING_VISIBILITY_DELAY_MS = 400;
export function useLogisticTasksFlow() {
    ...
    useCameraPrewarm("unified-scanner", 0, currentPageId === "logistic-tasks" && !isFullFeatureOpen, preferredScannerLensId, { attachPreview: true });
    ...
    const filtersRequestKey = useLogisticTasksStore(selectLogisticTasksFiltersRequestKey);
    ...
    useEffect(() => {
        if (!hasLoaded && !roleDefaultsAppliedRef.current) {
            ...
            const roleFilters = buildFiltersFromRoleDefaults(task_page_default_filters);
...
export function useLogisticTasksLoadingVisibilityFlow(isLoading, hasLoaded) {
...
export function useLogisticTasksReloadCallback() {
    return useCallback(() => {
        const { filters } = useLogisticTasksStore.getState();
        void logisticTasksActions.loadTasks(filters);
    }, []);
