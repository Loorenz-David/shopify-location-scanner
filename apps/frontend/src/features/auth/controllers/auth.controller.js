...
export async function registerController(payload) {
    const session = await registerApi(payload);
    tokenAuthController.setTokens(session.tokens);
    setStoredAuthUser(session.user);
    return session.user;
}
export async function hydrateAuthSessionController() {
...
export function clearAuthSessionController() {
    tokenAuthController.clearTokens();
    clearStoredAuthUser();
}
...
export async function appEnterController() {
    await appEnterApi();
}
export async function appLeaveController() {
    await appLeaveApi();
}
