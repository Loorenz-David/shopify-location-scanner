...
import { addLocationOptionsApi, deleteLocationOptionApi, } from "../api/set-location-options.api";
...
function reconcileLocationOptionOrder(serverOptions, preferredOrder) {
    const preferredOrderMap = new Map(preferredOrder.map((option, index) => [option.value.toLowerCase(), index]));
...
export async function hydrateLocationOptionsController() {
    ...
    try {
        ...
        store.setErrorMessage(apiMessage
...
export async function addLocationOptionController(value) {
    const store = useLocationOptionsSettingsStore.getState();
    ...
    const exists = previousOptions.some((option) => option.value.toLowerCase() === trimmed.toLowerCase());
    if (exists) {
    ...
    try {
        ...
        store.setOptions(reconcileLocationOptionOrder(response.metafield.options, optimisticOptions));
    }
    catch {
...
export async function removeLocationOptionController(value) {
    const store = useLocationOptionsSettingsStore.getState();
    const previousOptions = store.options;
    const optimisticOptions = previousOptions.filter((option) => option.value !== value);
    store.setSubmitting(true);
    store.setErrorMessage(null);
    store.setOptions(optimisticOptions);
