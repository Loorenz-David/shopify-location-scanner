...
export function UnifiedScannerProvider({ children, }) {
    const cameraFlow = useUnifiedScannerCameraFlow();
    const hasLocationOptions = useLocationOptionsStore((state) => state.options.length > 0);
    ...
    const isLookingUpItem = useUnifiedScannerStore((state) => state.isLookingUpItem);
    const itemLookupError = useUnifiedScannerStore((state) => state.itemLookupError);
    const locationWarningBanner = useUnifiedScannerStore((state) => state.locationWarningBanner);
    const lastPlacementError = useUnifiedScannerStore((state) => state.lastPlacementError);
    ...
    const availableLenses = useUnifiedScannerStore((state) => state.availableLenses);
    ...
    return (_jsx(UnifiedScannerPageProvider, { value: contextValue, children: children }));
}
