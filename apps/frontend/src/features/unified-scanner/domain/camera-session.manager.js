...
import { getScannerGuideRect, SCANNER_GUIDE_DEFAULT_ROI_PADDING_PX, } from "./scanner-guide.domain";
...
function getPrewarmHostId(id) {
    return `${CAMERA_REGION_IDS[id]}-prewarm-host`;
}
function getOrCreatePrewarmHostElement(id) {
...
function removePrewarmHostElement(id) {
...
async function waitForContainerToSettle(container, isCancelled) {
...
function ensureSessionVideoElement(session, container) {
    let video = session.videoElement instanceof HTMLVideoElement
...
function ensurePrewarmVideoElement(session) {
    return ensureSessionVideoElement(session, getOrCreatePrewarmHostElement(session.id));
}
function applyMobileVideoAttributes(video) {
...
function applyCameraVideoStyles(video) {
...
function buildVideoConstraints(resolvedDeviceId) {
    const video = {
        width: { ideal: 1280 },
        height: { ideal: 720 },
...
async function optimizeVideoTrackForQr(stream) {
...
function mapCssRectToVideoSourceRect(video, container, cssRect) {
...
function getCenteredSourceRect(video, ratio) {
    const vw = video.videoWidth;
    const vh = video.videoHeight;
...
function buildScanRegions(video, container, scanCount) {
    const containerRect = container.getBoundingClientRect();
    ...
    const guideSourceRect = mapCssRectToVideoSourceRect(video, container, guideRect);
    const regions = [];
    if (guideSourceRect) {
...
function drawSourceRectToCanvas(video, canvas, ctx, sourceRect) {
    const targetScale = Math.min(1, MAX_DECODE_CANVAS_EDGE_PX / Math.max(sourceRect.sw, sourceRect.sh));
    ...
    ctx.drawImage(video, Math.round(sourceRect.sx), Math.round(sourceRect.sy), Math.round(sourceRect.sw), Math.round(sourceRect.sh), 0, 0, targetWidth, targetHeight);
}
...
function isVideoRenderable(video, stream) {
    return (isStreamAlive(stream) &&
...
async function waitForPaintedVideoFrame(video, isCancelled, timeoutMs) {
...
async function waitForRenderableVideo(video, stream, isCancelled, timeoutMs) {
    const startedAt = performance.now();
    while (!isCancelled()) {
        if (isVideoRenderable(video, stream)) {
            const remainingMs = Math.max(250, timeoutMs - (performance.now() - startedAt));
            const gotPaintedFrame = await waitForPaintedVideoFrame(video, isCancelled, Math.min(remainingMs, 900));
...
async function prepareVideoForStream(video, stream, isCancelled, timeoutMs) {
    applyMobileVideoAttributes(video);
    applyCameraVideoStyles(video);
...
function stopStream(session, options = {}) {
...
function releaseOtherCameraSessions(activeId) {
...
async function selectBackCamera() {
    try {
        ...
        const backCamera = devices.find((device) => /back|rear|environment/i.test(device.label)) ?? devices[0];
...
async function openCameraStream(resolvedDeviceId) {
    try {
        return {
            stream: await navigator.mediaDevices.getUserMedia(buildVideoConstraints(resolvedDeviceId)),
            resolvedDeviceId,
        };
    }
    catch (error) {
        ...
        if (fallbackDeviceId && fallbackDeviceId !== resolvedDeviceId) {
            return {
                stream: await navigator.mediaDevices.getUserMedia(buildVideoConstraints(fallbackDeviceId)),
                resolvedDeviceId: fallbackDeviceId,
            };
        }
        return {
            stream: await navigator.mediaDevices.getUserMedia(buildVideoConstraints(undefined)),
            resolvedDeviceId: undefined,
        };
    }
}
async function attachPrewarmPreview(session) {
    ...
    session.hasRenderableVideo = await prepareVideoForStream(video, session.stream, () => session.prewarmCount === 0, PREWARM_PREVIEW_READY_TIMEOUT_MS);
}
async function startPrewarmStream(session) {
...
export function prewarmCameraSession(id, delayMs = 0, deviceId, options = {}) {
    const session = sessions[id];
...
export function attachDecodeSession(id, onDecode, onReady, deviceId, options = {}) {
    const session = sessions[id];
    let cancelled = false;
    cancelIdleTimer(session);
    ...
    async function start() {
        try {
            ...
            const streamAlive = prewarmStream !== null && isStreamAlive(prewarmStream);
            const prewarmDeviceId = streamAlive
                ? getStreamDeviceId(prewarmStream)
                : null;
            const canReuseStream = streamAlive &&
            ...
            let videoReady = await prepareVideoForStream(video, activeStream, () => cancelled, VIDEO_READY_TIMEOUT_MS);
            if (cancelled)
                return;
            if (!videoReady) {
                detachVideoStream(video);
                ...
                if (cancelled)
                    return;
                videoReady = await prepareVideoForStream(video, activeStream, () => cancelled, VIDEO_REATTACH_READY_TIMEOUT_MS);
            }
            if (cancelled)
                return;
            if (!videoReady) {
                ...
                session.stream = activeStream;
                void optimizeVideoTrackForQr(activeStream);
                videoReady = await prepareVideoForStream(video, activeStream, () => cancelled, VIDEO_REOPEN_READY_TIMEOUT_MS);
            }
            if (!videoReady) {
            ...
            await new Promise((r) => setTimeout(r, reusingStream ? 100 : 150));
            if (cancelled)
                return;
            ...
            const scheduleScanLoop = (delayMs) => {
                if (scanTimerId !== null) {
...
export function releaseAllCameraSessions() {
    for (const id of SESSION_IDS) {
        const session = sessions[id];
        cancelIdleTimer(session);
        cancelStartDelay(session);
...
export function suspendAllCameraSessions() {
    for (const id of SESSION_IDS) {
        const session = sessions[id];
        cancelIdleTimer(session);
        cancelStartDelay(session);
...
export function resumePrewarmedCameraSessions() {
    for (const id of SESSION_IDS) {
        const session = sessions[id];
