...
function isLogicalMultiLensCamera(label) {
    const normalized = label.toLowerCase();
    return (normalized.includes("dual") ||
...
function isRawSystemCamera(label) {
    return /^camera2\s+\d+/i.test(label.trim());
}
function deduplicateByLabel(devices) {
    const seen = new Set();
    return devices.filter((device) => {
        const key = device.label.toLowerCase().trim();
        if (!key)
            return true; // no label yet (pre-permission) — keep all
        if (seen.has(key))
            return false;
        seen.add(key);
        return true;
...
function toRearCameraDevices(cameraDevices) {
    const rearDevices = cameraDevices.filter((device) => !isFrontCamera(device.label));
    const hasLabels = rearDevices.some((d) => d.label.length > 0);
    if (!hasLabels) {
        // Pre-permission: labels are empty, cannot filter meaningfully.
        return rearDevices;
    }
    const filtered = deduplicateByLabel(rearDevices.filter((device) => !isLogicalMultiLensCamera(device.label) &&
        !isRawSystemCamera(device.label)));
    return filtered.length > 0 ? filtered : deduplicateByLabel(rearDevices);
}
function buildLensLabelByIndex(index, total) {
    if (total >= 3) {
...
export function mapCameraDevicesToLenses(cameraDevices) {
    const rearCameraDevices = toRearCameraDevices(cameraDevices);
    const sourceDevices = rearCameraDevices.length > 0 ? rearCameraDevices : cameraDevices;
    return sourceDevices.map((cameraDevice, index) => ({
        id: cameraDevice.id,
        label: buildLensLabelByIndex(index, sourceDevices.length),
    }));
}
export function getRememberedLensId() {
    if (typeof window === "undefined") {
        return null;
    }
    const value = window.localStorage.getItem(SCANNER_PREFERRED_LENS_ID_STORAGE_KEY);
    return value && value.length > 0 ? value : null;
}
export function rememberLensId(lensId) {
    if (typeof window === "undefined") {
        return;
    }
    window.localStorage.setItem(SCANNER_PREFERRED_LENS_ID_STORAGE_KEY, lensId);
}
export function resolvePreferredLensId(cameraDevices, selectedLensId, rememberedLensId) {
