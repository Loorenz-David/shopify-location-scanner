...
function shouldMuteLinkApiCall() {
    const rawFlag = import.meta.env["VITE_SCANNER_MUTE_LINK_API"];
    return String(rawFlag ?? "").toLowerCase() === "true";
}
function buildMutedLinkResponse(payload) {
    const firstItem = ("items" in payload ? payload.items[0] : payload);
    return {
        product: {
            id: firstItem.itemId,
...
function normalizeLinkItemPositionsResponse(payload, response) {
    if (!("results" in response)) {
        return response;
    }
    const firstRequestedItem = "items" in payload ? payload.items[0] : payload;
    const matchingSuccess = response.results.find((result) => result.ok &&
        result.idType === firstRequestedItem.idType &&
        result.itemId === firstRequestedItem.itemId);
    const firstSuccess = matchingSuccess ?? response.results.find((result) => result.ok);
    ...
    const failureResult = response.results.find((result) => result.idType === firstRequestedItem.idType &&
        result.itemId === firstRequestedItem.itemId);
...
export async function linkItemPositionsApi(payload) {
    if (shouldMuteLinkApiCall()) {
        return buildMutedLinkResponse(payload);
    }
    const response = await apiClient.patch("/shopify/items/location", payload, { requiresAuth: true });
