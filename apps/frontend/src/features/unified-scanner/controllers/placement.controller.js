...
export async function placementController() {
    ...
    if (location.mode === "shop") {
        const optimisticToken = itemScanHistoryActions.beginOptimisticLocationUpdate(item, location.code);
        try {
            ...
            itemScanHistoryActions.commitOptimisticLocationUpdate(optimisticToken, response);
            const storeAfterShop = useUnifiedScannerStore.getState();
            if (!storeAfterShop.selectedItem)
                return; // cycle was reset before API resolved
    ...
    if (!item.id) {
        store.setLastPlacementError("This item cannot be placed because it is missing its scan history ID.");
        store.setPhase("error");
        return;
    }
    const locationRecord = useLogisticLocationsStore
        .getState()
