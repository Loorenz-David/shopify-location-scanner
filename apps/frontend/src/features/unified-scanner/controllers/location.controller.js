...
import { UNIFIED_SCANNER_POPUP_IDS, } from "../types/unified-scanner.types";
export function applyLocationByValueController(value) {
    ...
    const location = resolveLocation(normalizedValue, store.locationMode, shopOptions, logisticLocations);
    if (!location) {
        store.setLocationWarningBanner(`Location "${normalizedValue}" not recognised.`);
        store.setPendingLocationValue(null);
        return;
    }
    applyResolvedLocationController(location);
}
export function applyResolvedLocationController(location) {
    const store = useUnifiedScannerStore.getState();
    ...
    if (warnings.length > 0) {
        ...
        store.setRequiresZoneMismatchAfterFixCheck(nextWarning?.type === "fix-check" &&
