...
import { selectItemScanHistoryFiltersRequestKey, useItemScanHistoryStore, } from "../stores/item-scan-history.store";
import { selectHomeShellCurrentPageId, selectHomeShellIsFullFeatureOpen, useHomeShellStore, } from "../../home/stores/home-shell.store";
...
export function useItemScanHistoryRealtimeFlow() {
    const lastRefreshByProductIdRef = useRef(new Map());
    const refreshHistoryItem = useCallback((productId) => {
        void itemScanHistoryActions.refreshHistoryItem(productId);
    }, []);
    const handleScanHistoryUpdated = useCallback((event) => {
        const normalizedProductId = event.productId.trim();
        if (!normalizedProductId) {
            return;
        }
        const now = Date.now();
        const lastRefreshAt = lastRefreshByProductIdRef.current.get(normalizedProductId) ?? 0;
...
export function useItemScanHistoryFlow() {
    ...
    useCameraPrewarm("unified-scanner", 0, currentPageId === "item-scan-history" && !isFullFeatureOpen, preferredScannerLensId, { attachPreview: true });
    const hasLoaded = useItemScanHistoryStore((state) => state.hasLoaded);
    const query = useItemScanHistoryStore((state) => state.query);
    const filtersRequestKey = useItemScanHistoryStore(selectItemScanHistoryFiltersRequestKey);
    const lastFetchedRequestKeyRef = useRef(null);
...
export function useItemScanHistoryLoadingVisibilityFlow(isLoading, hasLoaded) {
...
export function useItemScanHistoryPullRefreshFlow({ scrollContainerRef, }) {
    ...
    useEffect(() => {
        ...
        const handleTouchMove = (event) => {
            ...
            const nextDistance = Math.min(deltaY * PULL_REFRESH_RESISTANCE, PULL_REFRESH_MAX_PULL_PX);
