...
import { normalizeItemScanHistoryItem, normalizeItemScanHistoryPayload, } from "../domain/item-scan-history.domain";
...
export async function loadItemScanHistoryController(query, filters) {
...
export async function loadMoreItemScanHistoryController() {
    const store = useItemScanHistoryStore.getState();
...
export async function refreshItemScanHistoryItemController(productId) {
    ...
    try {
        ...
        const existingIndex = state.items.findIndex((item) => item.productId === normalizedProductId);
        if (!responseItem) {
            if (existingIndex < 0) {
                return;
            }
            const nextItems = state.items.filter((item) => item.productId !== normalizedProductId);
            useItemScanHistoryStore.setState({
                items: nextItems,
                total: Math.max(0, state.total - 1),
                expandedItemIds: state.expandedItemIds.filter((itemId) => itemId !== state.items[existingIndex]?.id),
            });
            return;
        }
        const normalizedItem = normalizeItemScanHistoryItem(responseItem);
        const matchesCurrentFilters = itemMatchesCurrentHistoryFilters(normalizedItem, state.query, state.filters);
        ...
        const remainingItems = state.items.filter((item) => item.productId !== normalizedProductId);
        useItemScanHistoryStore.setState({
            items: [normalizedItem, ...remainingItems],
            total: existingIndex >= 0
                ? state.total
            ...
            expandedItemIds: state.expandedItemIds.includes(normalizedItem.id)
                ...
                : state.expandedItemIds.filter((itemId) => itemId !== state.items[existingIndex]?.id),
        });
    }
    catch {
...
function itemMatchesCurrentHistoryFilters(item, query, filters) {
