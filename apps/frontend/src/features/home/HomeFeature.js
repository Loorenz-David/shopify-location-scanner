...
import { getVisibleBottomMenuItems, HOME_DEFAULT_PAGE_ID, } from "./domain/page-registry.domain";
import { useHomeShellFlow } from "./flows/use-home-shell.flow";
import { selectHomeShellCurrentPageId, selectHomeShellFullFeaturePageId, selectHomeShellIsFullFeatureOpen, selectHomeShellIsOverlayOpen, selectHomeShellIsPopupOpen, selectHomeShellOverlayTitle, selectHomeShellPopupPageId, selectHomeShellRegistry, useHomeShellStore, } from "./stores/home-shell.store";
import { HomeLayout } from "./ui/HomeLayout";
import { HomePage } from "./ui/HomePage";
import { useItemScanHistoryRealtimeFlow, useItemScanHistoryFlow, } from "../item-scan-history/flows/use-item-scan-history.flow";
...
import { selectTaskCount, useTaskCountStore, } from "../logistic-tasks/stores/task-count.store";
...
import { LazyAnalyticsPage, LazyLocationsSettingsPage, LazyScannerLogisticPlacementPage, LazyShopifySettingsPage, LazyStoreMapSettingsPage, LazyUsersSettingsPage, } from "./lazy-pages";
import { HomeIcon, SettingsIcon, StatsIcon, TaskIcon, } from "../../assets/icons";
export function HomeFeature({ onLogout }) {
    ...
    const registeredPages = useMemo(() => [
        ...
        },
        {
            id: "settings",
            title: "Settings",
            component: () => _jsx(SettingsFeature, { onLogout: onLogout }),
            bottomMenu: {
                label: "Settings",
                icon: SettingsIcon,
                slot: "right",
    ...
    const activePage = (currentPageId && registry[currentPageId]) ||
        registry[HOME_DEFAULT_PAGE_ID];
    if (!activePage) {
        return null;
    }
    const activeNavPageId = fullFeaturePageId ?? activePage.id;
    const navItems = getVisibleBottomMenuItems(registry, activeNavPageId).map((item) => item.id === "logistic-tasks" && taskCount > 0
        ? { ...item, count: taskCount }
        : item);
    const activeFullFeaturePage = (fullFeaturePageId && registry[fullFeaturePageId]) || null;
    return (_jsxs(_Fragment, { children: [_jsx(HomeLayout, { activePageTitle: activePage.title, ActivePageComponent: activePage.component, activeFullFeatureTitle: activeFullFeaturePage?.title ?? "Feature", ActiveFullFeatureComponent: activeFullFeaturePage?.component ?? null, isFullFeatureOpen: isFullFeatureOpen, navItems: navItems, isOverlayOpen: isOverlayOpen, overlayTitle: overlayTitle, overlayContent: _jsxs(_Fragment, { children: [_jsx(ItemScanHistoryOverlayHost, { onClose: homeShellActions.closeOverlayPage }), _jsx(LogisticTasksOverlayHost, { onClose: homeShellActions.closeOverlayPage })] }), isPopupOpen: isPopupOpen, popupContent: _jsxs(_Fragment, { children: [popupPageId === "placement-item-fixed-check" && (_jsx(PlacementItemFixedPopup, {})), popupPageId === "placement-zone-mismatch" && (_jsx(PlacementZoneMismatchPopup, {})), popupPageId === "unified-scanner-fix-check" && (_jsx(UnifiedFixCheckPopup, {})), popupPageId === "unified-scanner-zone-mismatch" && (_jsx(UnifiedZoneMismatchPopup, {}))] }), onClosePopup: homeShellActions.closePopupPage, onSelectPage: homeShellActions.selectNavigationPage }), isImageViewerOpen ? _jsx(ItemImageCarouselPage, {}) : null] }));
}
