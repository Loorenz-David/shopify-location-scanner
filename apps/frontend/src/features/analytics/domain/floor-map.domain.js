...
export function pct(value, axisPx) {
    return (value / 100) * axisPx;
}
export function pxToPercent(valuePx, axisPx) {
    if (axisPx <= 0) {
        return 0;
    }
    return clampPercent((valuePx / axisPx) * 100);
}
export function buildDefaultFloorBoundaryVertices(floorWidthCm, floorDepthCm) {
    return [
        { xCm: 0, yCm: 0 },
...
export function buildFloorMapViewportTransform(params) {
    ...
    const zoneBounds = zones.length > 0
        ? zones.reduce((current, zone) => {
        ...
        : null;
    const boundaryBounds = boundaryWorldVertices.length > 0
        ? boundaryWorldVertices.reduce((current, vertex) => ({
