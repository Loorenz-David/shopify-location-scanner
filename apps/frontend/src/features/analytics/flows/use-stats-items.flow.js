...
import { selectStatsItemsCurrentPage, selectStatsItemsIsLoading, selectStatsItemsIsOpen, selectStatsItemsQuery, selectStatsItemsRequestId, useStatsItemsStore, } from "../stores/stats-items.store";
    ...
    useEffect(() => {
        ...
        fetchStatsItemsController(query, currentPage).then((result) => {
            if (result.ok) {
                useStatsItemsStore
                    .getState()
                    .appendPage(result.items, result.total, result.page, capturedRequestId);
            }
            else {
