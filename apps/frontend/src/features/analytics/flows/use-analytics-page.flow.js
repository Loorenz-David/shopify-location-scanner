...
import { selectAnalyticsDateRange, selectAnalyticsVelocityChannel, useAnalyticsStore, } from "../stores/analytics.store";
export function useAnalyticsPageFlow() {
    ...
    const setLoadingOverview = useAnalyticsStore((state) => state.setLoadingOverview);
    ...
    const setChannelOverview = useAnalyticsStore((state) => state.setChannelOverview);
    const setVelocity = useAnalyticsStore((state) => state.setVelocity);
    const setVelocityCompareSeries = useAnalyticsStore((state) => state.setVelocityCompareSeries);
    ...
    const load = useCallback(async () => {
        ...
        try {
            const [zonesOverview, insights, categories, dimensions, channelOverview, timePatterns,] = await Promise.all([
