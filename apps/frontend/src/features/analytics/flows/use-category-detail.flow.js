...
import { selectAnalyticsCategoryDateRange, selectAnalyticsSelectedCategory, useAnalyticsStore, } from "../stores/analytics.store";
export function useCategoryDetailFlow() {
    ...
    const setCategoryDetail = useAnalyticsStore((state) => state.setCategoryDetail);
    const setCategoryTimePatterns = useAnalyticsStore((state) => state.setCategoryTimePatterns);
    const setLoadingCategoryDetail = useAnalyticsStore((state) => state.setLoadingCategoryDetail);
    useEffect(() => {
        if (!selectedCategory) {
            return;
        }
