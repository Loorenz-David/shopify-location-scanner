...
import { clampZoneShapeToBounds, hasZoneOverlap, } from "../domain/store-map-editor.domain";
import { useFloorPlanStore } from "../stores/floor-plan.store";
import { useFloorMapStore } from "../stores/floor-map.store";
import { computeGridSpacingCm, gridStepPx, snapToGridPx, } from "../utils/grid-utils";
function clampPercent(value) {
    return Math.max(0, Math.min(100, value));
}
function pxToPct(valuePx, axisPx) {
    if (axisPx <= 0) {
        return 0;
    }
...
function getPointerPosition(event, viewportTransform) {
...
export function useZoneEditorFlow(viewportTransform) {
    ...
    const beginDraftZone = useCallback((event) => {
        const stage = event.target.getStage();
    ...
    const updateDraftZone = useCallback((event) => {
        const stage = event.target.getStage();
    ...
    const commitDraftZone = useCallback(async () => {
        ...
        const type = zoneTypeInput?.trim().toLowerCase() === "corridor" ? "corridor" : "zone";
        const payload = {
            label: label.trim(),
            type,
    ...
    const renameZone = useCallback(async (zone) => {
    ...
    const saveZoneLabel = useCallback(async (zone, label) => {
    ...
    const saveZoneShape = useCallback(async (zone, shape) => {
    ...
    const saveZoneShapesBatch = useCallback(async (items) => {
        if (items.length === 0) {
            return;
        }
        const activeFloorPlan = getActiveFloorPlan();
        const payload = items.map(({ zone, shape }) => {
            const boundedShape = clampZoneShapeToBounds({
                ...zone,
                ...shape,
            });
    ...
    const createZone = useCallback(async (input) => {
        const activeFloorPlan = getActiveFloorPlan();
    ...
    const moveZone = useCallback(async (zone, xPx, yPx) => {
        const activeFloorPlan = getActiveFloorPlan();
        const viewportScale = viewportTransform?.scale ?? 1;
        let snappedX = xPx;
        let snappedY = yPx;
        if (activeFloorPlan) {
            const spacingCm = computeGridSpacingCm(viewportScale, stageWidth, activeFloorPlan.widthCm);
            snappedX = snapToGridPx(xPx, gridStepPx(spacingCm, stageWidth, activeFloorPlan.widthCm));
            snappedY = snapToGridPx(yPx, gridStepPx(spacingCm, stageHeight, activeFloorPlan.depthCm));
        }
        const nextZone = {
            ...zone,
    ...
    const removeZoneById = useCallback(async (zone) => {
    ...
    const normalizeSortOrder = useCallback(async () => {
        ...
        await reorderZonesApi(reorderedZones.map((zone) => ({
            id: zone.id,
