...
import { areFloorBoundaryVerticesEqual, buildCenteredDraftZone, buildDefaultFloorBoundaryVertices, buildEditorViewportTransform, findNonOverlappingDraftZone, hasShapeDraftChanges, hasUnsavedEditorChanges, } from "../domain/store-map-editor.domain";
import { useFloorMapStore } from "../stores/floor-map.store";
export function useStoreMapEditorSessionFlow({ zones, floorPlansCount, activeFloorPlan, selectedFloorPlanId, isEditorMode, isFloorBoundaryEditMode, floorBoundaryDraft, stageWidth, stageHeight, floorBoundaryVerticesWorld, setEditorMode, setFloorBoundaryDraft, setFloorBoundaryEditMode, resetShapeEditHistory, cancelBoundaryEdit, createFloorPlan, }) {
    const [selectedZone, setSelectedZone] = useState(null);
    const [activeZoneEditorMode, setActiveZoneEditorMode] = useState(null);
    ...
    const [editorBaseTransform, setEditorBaseTransform] = useState(null);
    ...
    const boundarySizeCm = useMemo(() => {
        ...
        return {
            widthCm: Math.max(0, floorBoundaryDraft[1].xCm - floorBoundaryDraft[0].xCm),
            depthCm: Math.max(0, floorBoundaryDraft[3].yCm - floorBoundaryDraft[0].yCm),
        };
    }, [floorBoundaryDraft]);
    const hasPendingBoundaryChanges = useMemo(() => {
        ...
        const persistedBoundary = activeFloorPlan.shape ??
    ...
    useEffect(() => {
        ...
        setEditorBaseTransform(buildEditorViewportTransform({
            zones,
            stageWidth,
            stageHeight,
    ...
    const resolvedSelectedZone = useMemo(() => selectedZone
        ? (stagedZoneEdits[selectedZone.id] ?? selectedZone)
        : null, [selectedZone, stagedZoneEdits]);
    useEffect(() => {
        setLabelDraft(resolvedSelectedZone?.label ?? "");
        setActiveZoneEditorMode((current) => resolvedSelectedZone ? (current ?? "menu") : null);
    ...
    const editorZonesBase = useMemo(() => zones.map((zone) => stagedZoneEdits[zone.id] ?? zone), [stagedZoneEdits, zones]);
    const editorZones = isDraftZone && resolvedSelectedZone
        ? [...editorZonesBase, resolvedSelectedZone]
        : editorZonesBase;
    const hasPendingShapeChanges = hasShapeDraftChanges(resolvedSelectedZone, shapeDraft);
    ...
    const confirmDiscardChanges = useCallback((message = "Discard the current editor changes?") => {
    ...
    const beginCreateZone = useCallback((viewportTransform) => {
        const nextDraft = findNonOverlappingDraftZone(buildCenteredDraftZone({
            stageWidth,
            stageHeight,
    ...
    const clearStagedZoneEdit = useCallback((zoneId) => {
        if (!zoneId) {
            return;
        }
