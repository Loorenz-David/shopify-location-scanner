...
interface CategoryRankingComparisonProps {
...
function SortArrowsIcon({ sortKey }: { sortKey: SortKey }) {
  return (
    ...
    >
      ...
      <path
        d="M7 17l4-5H3l4 5Z"
        fill={sortKey === "totalRevenue" ? "#059669" : "#cbd5e1"}
      />
    </svg>
  );
}

export function CategoryRankingComparison({
  data,
  onCategoryClick,
}: CategoryRankingComparisonProps) {
  ...
  return (
    <div className="w-full max-h-64 overflow-y-auto">
      ...
      <div className="sticky top-0 z-10 flex items-center gap-3 bg-white px-1 pb-1.5 border-b border-slate-100">
        ...
        <span
          className={`flex-1 text-[10px] font-semibold uppercase tracking-[0.07em] transition-colors ${
            sortKey === "totalRevenue" ? "text-emerald-600" : "text-slate-400"
          }`}
        >
          Revenue
        </span>
