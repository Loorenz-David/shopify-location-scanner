...
import { filterLogisticLocations, sortWithRecentFirst, } from "../../logistic-locations/domain/logistic-locations.domain";
...
export function useLocationsSettingsFlow(query) {
    useLocationOptionsSettingsFlow();
    useLogisticLocationsFlow();
    const options = useLocationOptionsSettingsStore((state) => state.options);
    const shopIsLoading = useLocationOptionsSettingsStore((state) => state.isLoading);
    const shopIsSubmitting = useLocationOptionsSettingsStore((state) => state.isSubmitting);
    const shopError = useLocationOptionsSettingsStore((state) => state.errorMessage);
    ...
    const logisticIsLoading = useLogisticLocationsStore((state) => state.isLoading);
    const logisticIsSubmitting = useLogisticLocationsStore((state) => state.isSubmitting);
    const logisticError = useLogisticLocationsStore((state) => state.errorMessage);
    const combinedList = useMemo(() => {
        const filteredLogistic = sortWithRecentFirst(filterLogisticLocations(rawLocations, query), recentlyAddedIds);
        const filteredShop = filterLocationOptions(options, query);
        const logisticItems = filteredLogistic.map((loc) => ({
            kind: "logistic",
            id: loc.id,
            location: loc.location,
            zoneType: loc.zoneType,
        }));
        const shopItems = filteredShop.map((opt) => ({
            kind: "shop",
            label: opt.label,
            value: opt.value,
        }));
    ...
    const showPickerCondition = query.trim().length > 0 &&
