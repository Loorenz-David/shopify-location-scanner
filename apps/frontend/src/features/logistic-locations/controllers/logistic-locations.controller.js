...
export async function hydrateLogisticLocationsController() {
...
export async function createLogisticLocationController(location, zoneType) {
    const store = useLogisticLocationsStore.getState();
    const currentLocations = store.locations;
    const isDuplicate = currentLocations.some((l) => l.location.toLowerCase() === location.trim().toLowerCase());
    if (!location.trim() || isDuplicate) {
        store.setErrorMessage(isDuplicate
    ...
    try {
        ...
        store.setLocations(useLogisticLocationsStore
            .getState()
...
export async function deleteLogisticLocationController(id) {
    const store = useLogisticLocationsStore.getState();
    const currentLocations = store.locations;
