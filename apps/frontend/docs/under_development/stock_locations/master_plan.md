...
| P8 | PDF data assembly + delivery | Codex | IMPLEMENTED | 2026-09-02 | coordinator | **coordinator verification complete, APPROVED waits on one owner card** (summary-count semantics for excluded states, MC10 "full counts"); perimeter = declared (7 paths), foreign-diff digest matches the stamp; 146 tests (133 → 145 by Codex, +1 coordinator row); M1 run and re-planted in a second shape (`locations` only) plus three probes, each reddening its row; **one S10 hole found and repaired** — the initializer's `propertyKeyOrder` was unobserved (12/12 green with it deleted), `C5(keyOrder)` added and proved; H1 folded as C8 (S7), preview as C7(d); lint 0 in perimeter |
