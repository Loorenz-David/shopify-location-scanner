...
const normalizeImageList = (value: string | null | undefined): string | null => {
  const urls = (value ?? "")
    .split(",")
...
const summarizeImages = (value: string | null): {
  count: number;
  first: string | null;
} => {
  const urls = (value ?? "")
  ...
  if (shops.length > 1) {
    throw new Error(
      "Multiple linked shops found. Set SHOP_ID to specify which one:\n" +
        shops.map((shop) => `  SHOP_ID=${shop.id}  (${shop.shopDomain})`).join("\n"),
    );
  }
...
const main = async (): Promise<void> => {
  ...
  for (const record of records) {
    ...
    try {
      ...
      if (currentImageUrl === nextImageUrl && currentQuantity === nextQuantity) {
        unchanged += 1;
        continue;
      }
