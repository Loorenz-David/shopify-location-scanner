...
const handleFallbackCache = new Map<string, string | null>();
const inFlightHandleFallbacks = new Map<string, Promise<string | null>>();

const HANDLE_PATH_REGEX = /\/(?:[a-z]{2}(?:-[a-z]{2})?\/)?products\/([^/?#]+)/i;

const normalizeShopDomain = (shopDomain: string): string | null => {
