...
const isRelativeSqlitePath = (databaseUrl) => {
...
if (env.NODE_ENV === "production" && isRelativeSqlitePath(env.DATABASE_URL)) {
    throw new Error("In production, DATABASE_URL for SQLite must use an absolute path (example: file:/var/lib/item-scanner/data/app.db).");
}
