...
const mapFrontendFieldToColumn = (field) => {
    switch (field) {
        case "username":
            return "username";
        case "sku":
...
export const GetScanHistoryQuerySchema = z
    ...
    .transform((input) => {
    const stringColumns = input.fields?.map(mapFrontendFieldToColumn) ?? input.stringColumns;
    let sold = input.sold;
...
//# sourceMappingURL=scan-history.contract.js.map